...
def run_agentic_rag():
    ...
    def create_chroma_index_node(state: State) -> Dict[str, Any]:
        """Create a Chroma index if possible, otherwise build an in-memory index fallback.
        The in-memory index stores embeddings and chunk metadata in session_state['inmem_index'].
        """
        try:
            ...
            if not chunks or not state.get("embeddings_model"):
                # ensure we still record chunks so fallback retrieval can use them
            ...
            try:
                # Try building Chroma-backed vector store
                ...
                # Fall back: create an in-memory index
                try:
                    emb_model = state["embeddings_model"]
                    X = embed_texts_cached(emb_model, texts, user_id, f"{file_hash}_chunks")
                    st.session_state["inmem_index"] = {
                        "embeddings": X,
                        "texts": texts,
                        "metadatas": metadatas
                    }
                    ...
                    print(f"create_chroma_index_node fallback error: {e}")
                    st.session_state["chunk_store"] = chunks
                    return {"vector_store": None}
        except Exception as e:
    ...
    def retrieve_node(state: State) -> Dict[str, Any]:
        # Primary: use vector_store retriever if available
        if state.get("vector_store") is not None:
            try:
                ...
                print(f"retrieve_node vector_store error: {e}")
        # Fallback: use in-memory index if present
        try:
            inmem = st.session_state.get("inmem_index")
            chunks = st.session_state.get("chunk_store", [])
            if inmem and len(inmem.get("embeddings", [])) > 0:
                emb_model = state.get("embeddings_model")
                q_emb = embed_texts_wrap(emb_model, [state.get("question", "")])[0]
                # cosine similarity
                E = inmem["embeddings"]
                # guard shapes
                if E.ndim == 1:
                    E = E.reshape(1, -1)
                norms = np.linalg.norm(E, axis=1) * (np.linalg.norm(q_emb) + 1e-12)
                sims = np.dot(E, q_emb) / (norms + 1e-12)
                # get top k
                k = min(6, E.shape[0])
                idxs = np.argsort(-sims)[:k]
                out = []
                for i in idxs:
                    meta = inmem["metadatas"][i]
                    txt = inmem["texts"][i]
                    out.append({"text": txt, "metadata": meta, "score": float(sims[i])})
                return {"retrieved_docs": out}
        except Exception as e:
            print(f"retrieve_node inmem error: {e}")
        # Last fallback: no retrieval
...
# SECOND HALF: Streamlit UI and orchestration (place after part1 in the same file to form app.py)
...
# The following code expects all helper functions, classes, and run_agentic_rag
# to be defined (i.e., place this file after part1 when concatenating into app.py).

# ---------- main app UI ----------

def main():
    # nest_asyncio applied in part1; safe to call again
    try:
        import nest_asyncio
        nest_asyncio.apply()
    except Exception:
    ...
    if not GEMINI_API_KEY or not TAVILY_API_KEY:
        st.error("API keys for Gemini or Tavily are not configured. Set GEMINI_API_KEY and TAVILY_API_KEY in environment or Streamlit secrets.")
        st.stop()
...
# Notes:
# - Concatenate corrected part1 then this corrected part2 into a single app.py for deployment.
# - Ensure GEMINI_API_KEY and TAVILY_API_KEY are set in environment or Streamlit secrets.
# - Install required dependencies before running: streamlit, PyPDF2, python-dotenv, nest_asyncio, numpy, (optional: tiktoken, scikit-learn, joblib, pdf2image, pytesseract, colpali_engine, langchain packages)
