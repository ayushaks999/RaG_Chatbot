...
# Paste this file first when combining into a single app.py
...
# LLM / vectorstore wrappers (graceful fallback)
try:
...
# Secrets helper: prefer st.secrets then env
...
# ---------- create index (Chroma) with fallback ----------
...
# ---------- retrieve node (supports Chroma or in-memory) ----------
...
# ---------- rerank, scoring, evaluate, generate, summarize ----------
...
def web_search_node(state: Dict[str, Any]) -> Dict[str, Any]:
    hits = []
    try:
        results = TavilySearchResults(max_results=4, tavily_api_key=TAVILY_API_KEY).invoke({"query": state["question"]}) if TavilySearchResults is not None else []
...
def run_agentic_rag():
    if StateGraph is None:
        raise RuntimeError("langgraph.graph.StateGraph not available in environment")
...
# Paste this file after part1 when combining into a single app.py
...
# This UI expects backend functions defined in part1 (ensure_models, run_agentic_rag, save_uploaded_file, persist_chat, load_user_chats, train_reranker_incremental, check_rate_limit, _short_hash_bytes_multiple, _db_conn, get_user_model_dir, list_user_models, load_user_reranker, save_uploaded_model_file, MAX_UPLOAD_BYTES)
...
if not GEMINI_API_KEY or not TAVILY_API_KEY:
    st.error("API keys for Gemini or Tavily are not configured. Add them to Streamlit secrets or env vars.")
    st.stop()
...
# ColPali controls
sidebar.markdown("---")
colpali_enabled = sidebar.checkbox("Enable ColPali hybrid rerank (experimental)", value=False)
        ...
        if not user_query:
            ...
            if current_user is None:
                ...
                with st.spinner("Thinking..."):
                    # initialize models (ensures keys present)
                    ...
                    try:
                        ...
                        # run the RAG graph
                        import asyncio
                        final_state = asyncio.run(st.session_state["rag_app"].ainvoke(initial_state))
    ...
    if st.button("Summarize documents"):
        ...
        if not docs:
            ...
            with st.spinner("Summarizing documents..."):
                ...
                initial_state = {"documents": docs, "question": "summarize", "llm": st.session_state["llm"], "embeddings_model": st.session_state["embeddings_model"], "file_hash": file_hash, "user_id": current_user}
                st.session_state["last_file_hash"] = file_hash
