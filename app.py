# app_full_parts.py
# Single file containing TWO parts separated by a clear marker.
# Part A (backend, QnA pipeline, DB, indexing, reranker helpers)
# --- PART A START ---
import os
import io
import re
...
# ColPali index (optional) -----------------------------------------------------
class ColpaliIndex:
    def __init__(self, model_name: str, user_id: int, file_hash: str):
        self.model_name = model_name
        self.user_id = user_id
        self.file_hash = file_hash
        self.device = 'cuda' if (torch and torch.cuda.is_available()) else 'cpu' if 'torch' in globals() else 'cpu'
        self.model = None
    ...
    def _load_model(self):
        ...
        try:
            ...
            elif self.model_name.lower().endswith('smol'):
...
def load_and_chunk_docs_node(docs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    # returns list of chunks with provenance
    out_chunks = []
            ...
            for p, page in enumerate(reader.pages):
                ...
                if not page_text.strip() and PYTESS_AVAILABLE:
                    try:
                        ...
                        pass
                out_chunks.append({'text': page_text, 'doc_id': i, 'filename': doc.get('filename', f'doc_{i}'), 'page': p+1})
        except Exception:
    ...
    for c in out_chunks:
        ...
        if not t:
            ...
            continue
        for start in range(0, len(t), chunk_size - overlap):
            combined.append({'text': t[start:start+chunk_size], 'doc_id': c['doc_id'], 'filename': c['filename'], 'page': c['page']})
    return combined

def create_chroma_index_node(chunks: List[Dict[str, Any]], emb_model, user_id: str, file_hash: str):
    if not chunks or emb_model is None:
        return None
    ...
    try:
        ...
        try:
            vs = Chroma.from_texts(texts=texts, embedding=emb_model, metadatas=metadatas, persist_directory=persist_dir)
        except TypeError:
            vs = Chroma.from_texts(texts=texts, embedding=emb_model, metadatas=metadatas)
        try:
        ...
        # fallback: compute and store embeddings in memory
        X = embed_texts_cached(emb_model, texts, user_id, file_hash + '_fallback')
        st.session_state['inmem_index'] = {'embs': np.array(X), 'texts': texts, 'metadatas': metadatas}
        return None

# retrieval --------------------------------------------------------------------
def retrieve_node(vector_store, emb_model, question: str, k: int = 6):
    ...
    if not inmem:
        return []
    q = embed_texts_wrap(emb_model, [question])[0]
    data = np.array(inmem['embs'])
    dots = data.dot(q)
    norms = (np.linalg.norm(data, axis=1) * (np.linalg.norm(q) + 1e-12))
...
def train_reranker_incremental(user_id: int):
    ...
    try:
        ...
        if joblib:
            ...
            st.warning('joblib not available; trained model not saved')
    except Exception as e:
        st.error(f'Failed to train reranker incrementally: {e}')

# generate answer ----------------------------------------------------------------
def build_context(retrieved: List[Dict[str, Any]], top_k: int = 4) -> str:
    parts = []
    for i, r in enumerate(retrieved[:top_k]):
        meta = r.get('metadata', {})
        fn = meta.get('filename', 'unknown')
        page = meta.get('page', '?')
        parts.append(f"[[SOURCE #{i+1}] {fn} (p.{page})]\n{r.get('text','').strip()}\n")
    return '\n---\n'.join(parts)

def generate_answer(llm, retrieved: List[Dict[str, Any]], question: str) -> str:
    context = build_context(retrieved, top_k=4)
    if not context.strip():
        return "I couldn't find any relevant information in the uploaded documents."
    prompt = (
        "You are an assistant that MUST use ONLY the CONTEXT blocks below to answer. "
        "If the information is present, answer concisely and cite the source by number. "
        "If not present, say 'I couldn't find that information in the provided documents.'\n\n"
        f"CONTEXT:\n{context}\n\nQUESTION: {question}\n\nAnswer:"
    )
    try:
...
def web_search(question: str) -> List[str]:
    try:
        if TavilySearchResults is None:
            return []
        tool = TavilySearchResults(max_results=4, tavily_api_key=TAVILY_API_KEY)
...
class SimpleRAG:
    ...
    def invoke(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        llm = state.get('llm')
        emb = state.get('embeddings_model')
        user_id = state.get('user_id', 'anon')
        file_hash = state.get('file_hash', 'nohash')

        chunks = load_and_chunk_docs_node(docs)
        state['text_chunks'] = chunks
        vector_store = create_chroma_index_node(chunks, emb, user_id, file_hash)
        state['vector_store'] = vector_store
        retrieved = retrieve_node(vector_store, emb, question, k=6)
        state['retrieved_docs'] = retrieved

        if not retrieved:
            state['web_results'] = web_search(question)
            if state.get('web_results'):
                # use web text as context
                web_context = '\n\n'.join(state['web_results'][:3])
                # pretend we retrieved this as a single snippet
                fake = [{'text': web_context, 'metadata': {'source': 'web'}}]
                ans = generate_answer(llm, fake, question)
                state['answer'] = ans
                state['confidence'] = 0.4
                return state
            state['answer'] = "I couldn't find any relevant information in the uploaded documents."
            state['confidence'] = 0.0
            return state

        ans = generate_answer(llm, retrieved, question)
        state['answer'] = ans
        if "couldn't find" in ans or "I can't find" in ans or "I couldn't find" in ans:
            state['confidence'] = 0.0
        else:
...
async def get_rag_response(rag_app, initial_state):
    if hasattr(rag_app, 'ainvoke'):
        return await rag_app.ainvoke(initial_state)
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, lambda: rag_app.invoke(initial_state))
...
# --- PART B START (Streamlit UI) ---

# Streamlit UI requires nest_asyncio for some environments
nest_asyncio.apply()
...
def main_ui():
    ...
    if not GEMINI_API_KEY:
        st.error('GEMINI_API_KEY not configured. Add it to Streamlit secrets or env vars.')
        st.stop()
                    ...
                    with st.spinner('Thinking...'):
                        ...
                        if 'llm' not in st.session_state or 'embeddings_model' not in st.session_state:
                            if ChatGoogleGenerativeAI is None or GoogleGenerativeAIEmbeddings is None:
                                st.error('LLM / Embeddings classes not available in this environment.')
                                st.stop()
        ...
        if st.button('Summarize documents'):
            ...
            if not docs:
                ...
                with st.spinner('Summarizing documents...'):
                    ...
                    initial_state = {'documents': docs, 'question': 'summarize', 'llm': st.session_state.get('llm'), 'embeddings_model': st.session_state.get('embeddings_model'), 'file_hash': file_hash, 'user_id': current_user}
                    st.session_state['last_file_hash'] = file_hash
...
# helper functions used by UI for model files
